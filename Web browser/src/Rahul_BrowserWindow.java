...
/**
 * This package-private class defines one Rahul_browser window for use with 
 ...
 */
class Rahul_BrowserWindow extends Stage {

    private Rahul_WebBrowser Rahul_owner;    // the WebBrowser application
    private WebEngine Rahul_webEngine; // loads and manages pages
    private Menu Rahul_windowMenu;     // holds window-related commands

    /**
     * The constructor sets up the window, and starts loading the
     * initial web page location, if given.  It does not show the window.
     * @param Rahul_browser     the WebBrowser application that is createing this window
     * @param Rahul_initialURL  if non-null, the window will attempt to load this URL
     */
      Rahul_BrowserWindow(Rahul_WebBrowser Rahul_browser, String Rahul_initialURL) {
        
        Rahul_owner = Rahul_browser;
        ...
        Rahul_webEngine = webview.getEngine();
        ...
        Rahul_webEngine.locationProperty().addListener( (o,oldVal,newVal) -> {
        ...
        Rahul_webEngine.titleProperty().addListener( (o,oldVal,newVal) -> {
            if (newVal == null)
                setTitle("Untitled " + Rahul_owner.Rahul_getNextUntitledCount());
            else
                setTitle(newVal); 
        });
        ...
        Rahul_webEngine.getLoadWorker().stateProperty().addListener( (o,oldVal,newVal) -> {
        ...
        cancelButton.setOnAction( e -> { // Cancel the ongoing page load
            if ( Rahul_webEngine.getLoadWorker().getState() == Worker.State.RUNNING )
                Rahul_webEngine.getLoadWorker().cancel();
        ...
        Rahul_webEngine.setOnAlert( evt -> SimpleDialogs.message(evt.getData(), "Alert from web page") );
        Rahul_webEngine.setPromptHandler( promptData -> 
                    SimpleDialogs.prompt(promptData.getMessage(), 
                                              "Query from web page", promptData.getDefaultValue()));
        Rahul_webEngine.setConfirmHandler( str -> 
                    SimpleDialogs.confirm(str, "Confirmation Needed").equals("yes") );
        
        /* If the Rahul_initialURL is not null, load that page! */
        
        if (Rahul_initialURL != null) {
               // load the initial web page, if any
            doLoad(Rahul_initialURL);
        }
    ...
    private MenuBar makeMenuBar() {
        ...
        newWin.setOnAction( e -> Rahul_owner.Rahul_newBrowserWindow(null) );
        ...
        open.setOnAction( e -> {
            ...
            if (url != null && url.trim().length() > 0)
                Rahul_owner.Rahul_newBrowserWindow(url);
        });
        Rahul_windowMenu = new Menu("Window");
        Rahul_windowMenu.getItems().addAll(newWin,close,open,new SeparatorMenuItem());
        Rahul_windowMenu.setOnShowing( e -> populateWindowMenu() );
        MenuBar menubar = new MenuBar(Rahul_windowMenu);
        return menubar;
    }
    ...
    private void populateWindowMenu() {
        ArrayList<Rahul_BrowserWindow> windows = Rahul_owner.getOpenWindowList();
        while (Rahul_windowMenu.getItems().size() > 4) {
            ...
            Rahul_windowMenu.getItems().remove(Rahul_windowMenu.getItems().size() - 1);
        }
        if (windows.size() > 1) {
            ...
            Rahul_windowMenu.getItems().add(item);
            Rahul_windowMenu.getItems().add( new SeparatorMenuItem() );
        }
        for (Rahul_BrowserWindow window : windows) {
            ...
            final Rahul_BrowserWindow win = window; // (for use in a lambda expression)
